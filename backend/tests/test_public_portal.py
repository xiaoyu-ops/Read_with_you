...
class PublicPortalTest(unittest.TestCase):
    ...
    def test_portal_home_shows_fixed_product_demo_and_local_core_entry(self) -> None:
        ...
        self.assertIn("贴合度会低于真实本地工作台", home.text)
        self.assertIn("会直接翻译你实际选中的具体内容", home.text)
        self.assertIn("真实本地工作台会直接翻译具体选区", home.text)
    ...
    def test_minimal_preview_is_separate_and_public_portal_only(self) -> None:
        ...
        for mode in ("self_hosted", "local_core"):
            with TestClient(api_main.create_app(mode)) as client:
                self.assertEqual(client.get("/minimal-preview").status_code, 404)
