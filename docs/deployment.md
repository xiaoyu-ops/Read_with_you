...
| 状态 | 只能证明 | 不能证明 |
|---|---|---|
| commit 已推送 | 远端仓库包含新代码 | 生产域名已更新 |
| Core 候选 workflow 成功 | 私有安装候选可构建 | 公网主页已部署或公开 Release 已创建 |
| 门户候选端口通过 | 新门户镜像可运行且边界正确 | 正式端口已切换 |
| 正式容器 healthy | 生产进程和健康检查通过 | Cloudflare 与用户浏览器已看到新页面 |
| 正式域名和真实浏览器通过 | 用户可见版本已更新 | 未签名 Core 可以公开下载 |

任何公网主页修改都必须依次完成：

1. **源状态**：确认工作树、目标 commit、GitHub `main` SHA 与归档 SHA-256；归档
   排除 `.env`、真实数据、凭据、当前任务和内部开发历史。
2. **隔离候选**：在非生产 loopback 端口启动新镜像，检查 `/health`、
   新增文案、应删除的旧文案、固定素材类型/缓存，以及论文、配置、Agent、翻译、
   OpenAPI、`/assets` 和路径穿越全部保持 404。
3. **生产切换**：只使用已通过候选验收的镜像重建正式门户，继续挂载原门户数据；
   旧镜像与上一 release 目录保留用于回滚。
4. **四项最终证据**：同时记录正式镜像 tag、容器 `healthy`、Cloudflare 域名响应
   和真实浏览器标题/H1。四项缺一，不得汇报“已同步”“已上线”或“已收尾”。
5. **任务收口**：生产复核后移除隔离候选容器，立即更新开发历史并清空当前任务。

工具读取 GitHub 日志、浏览器连接或外部 TLS 请求出现 timeout、`system error`
时，只能描述为“检查工具/网络失败”。在直接检查生产容器、`/health` 或正式页面
之前，不得把它归因于产品、Core 或公网服务故障。反过来，本地 `8551` 预览成功也
不能作为正式域名通过的证据。
