...
按下方 GitHub 源码流程启动本地 Core，再访问公网入口或直接打开 `127.0.0.1:8520`。Chrome 首次从公网入口检查 Core 时可能请求本地网络访问权限；它只用于连接本机 loopback 服务。若浏览器拦截探测，可直接使用页面上的“尝试打开本地工作台”。本地首页可输入论文标题、arXiv ID 或 URL，确认检索候选并查看关系图谱，也可导入本地 PDF。
