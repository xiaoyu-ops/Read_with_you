...
> 公网入口可直接检索公开论文并查看论文关系图谱；原始 PDF 阅读、翻译、笔记、Pet、Agent 和 API Key 仍只在用户自己的 Pet Core 中处理。
...
在公网或本地首页输入论文标题、arXiv ID 或 URL，确认检索候选并查看关系图谱；导入本地 PDF 与打开阅读器需要本地 Core。
...
- 公网门户只处理公开学术元数据检索和可重建的论文关系图，不接收 PDF、翻译、笔记、问题、回答、Key 或本地文件。
