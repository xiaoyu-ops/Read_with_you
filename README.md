...
> 公网入口会先检查这台电脑上的本地 Core；未运行时引导到 GitHub 安装或启动，已就绪时再打开本地工作台。原始 PDF、翻译、笔记、Pet、Agent 和 API Key 始终只在用户自己的 Pet Core 中处理。
...
按下方 GitHub 源码流程启动本地 Core，再访问公网入口或直接打开 `127.0.0.1:8520`。本地首页可输入论文标题、arXiv ID 或 URL，确认检索候选并查看关系图谱，也可导入本地 PDF。
...
- 公网门户首页只探测本地 Core 的无内容就绪状态并提供 GitHub 安装入口；它保留的公开学术元数据与论文关系图 API 不接收 PDF、翻译、笔记、问题、回答、Key 或本地文件。
